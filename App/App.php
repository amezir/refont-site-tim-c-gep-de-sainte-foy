        ...
        if ($controleur === 'site'){
            ...
            switch ($action) {
                ...
                case 'apropos':
                    ...
                    break;
                case 'stages':
                    $this->monControleur->stages();
                        break;
                case 'formation':
                    $this->monControleur->formation();
                    break;
                case 'pieddepage':
                    $this->monControleur->pieddepage();
                    break;
                default:
                    echo 'Erreur 404 !';
            }
        }else{
            if ($controleur === 'message'){
                $this->monControleur = new ControleurMessage();
                switch ($action) {
                    case 'creer':
                        $this->monControleur->creer();
                        break;
                    case 'insert':
                        $this->monControleur->insert();
                        break; 
                    default:
                        echo 'Erreur 404 !';
                    }
                }else{
                        if ($controleur === 'projets'){
                            $this->monControleur = new ControleurProjets();
                            switch ($action) {
                                case 'projets':
                                    $this->monControleur->projets();
                                    break;
                                case 'filtre':
                                    $this->monControleur->filtre();
                                    break;
                                case 'projets_details':
                                    $this->monControleur->projets_details();
                                    break;
                                case 'envoyerCourriel':
                                    $this->monControleur->envoyerCourriel();
                                    break;
                                default:
                                    echo 'Erreur 404 !';
                        }
